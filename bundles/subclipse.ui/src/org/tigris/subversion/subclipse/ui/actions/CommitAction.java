...
/**
 * Action for checking in files to a subversion provider Prompts the user for a
 * release comment, and shows a selection list of added and modified resources,
 * including unversioned resources. If selected, unversioned resources will be
 * added to version control, and committed.
 */
				...
				public void run(IProgressMonitor monitor) throws InvocationTargetException {
					try {
						...
						if (modified.length == 0) {
							MessageDialog.openInformation(getShell(), Policy.bind("CommitDialog.title"),
									Policy.bind("CommitDialog.noChanges")); // $NON-NLS-1$ //$NON-NLS-2$
							commit = false;
						} else {
		...
		case IMarker.SEVERITY_WARNING:
			...
			if (MessageDialogWithToggle.PROMPT.equals(allowCommitsWithWarnings)
				...
				MessageDialogWithToggle warningDialog = MessageDialogWithToggle.openYesNoQuestion(shell,
						Policy.bind("CommitWizard.commitResources"), Policy.bind("CommitWizard.warningMarkers"),
						Policy.bind("CommitWizard.warningQuestion"), false, preferenceStore,
						ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_WARNINGS); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		...
		case IMarker.SEVERITY_ERROR:
			...
			if (MessageDialogWithToggle.PROMPT.equals(allowCommitsWithErrors)
				...
				MessageDialogWithToggle errorDialog = MessageDialogWithToggle.openYesNoQuestion(shell,
						Policy.bind("CommitWizard.commitResources"), Policy.bind("CommitWizard.errorMarkers"),
						Policy.bind("CommitWizard.errorQuestion"), false, preferenceStore,
						ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_ERRORS); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
						...
						public boolean visit(IResource aResource) {
							...
							try {
								...
								if (svnResource.isManaged())
									return true;
								if ((aResource.getType() == IResource.FOLDER) && isSymLink(aResource)) // don't traverse
																										// into symlink
																										// folders
									return false;
